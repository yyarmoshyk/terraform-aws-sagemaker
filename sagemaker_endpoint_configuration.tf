...
resource "aws_sagemaker_endpoint_configuration" "sagemaker_endpoint_configuration" {
  ...
  name = var.sagemaker_endpoint_configuration_name != "" ? "${lower(var.sagemaker_endpoint_configuration_name)}-${random_integer.random.id}" : "${lower(var.name)}-sagemaker-endpoint-conf-${lower(var.environment)}-${random_integer.random.id}"
